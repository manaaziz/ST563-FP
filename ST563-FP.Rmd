...
rf.class.pre <- randomForest(model.class.pre, data=wine, ntree=500, mtry=3, importance=TRUE)
...
vimp.class <- ggplot(VIrf.pre.class.sort[-12,],aes(x=X,y=MeanDecreaseAccuracy))+
...
# Fit LASSO model and plot result
lm.lasso <- glmnet::cv.glmnet(X, wine$quality, alpha = 1)
lambda <- lm.lasso$lambda.min
plot(lm.lasso)

# Fit LASSO model with best tune
lasso.best <- glmnet(X, wine$quality, alpha = 1)
predict(lasso.best, s = lambda, type = "coefficients")

# Create model and get summary output
lm.sub <- regsubsets(quality ~ ., data = wine, nvmax = 10)
smre <- lm.sub %>% summary()

# Plot the Cp, BIC, and R^2 results in one plot
par(mfrow = c(2,2))
...
```{r form}
form.reg <- quality ~ volatile.acidity + chlorides + 
            total.sulfur.dioxide + pH + sulphates + alcohol
form.class <- class ~ alcohol + sulphates + volatile.acidity + density + total.sulfur.dioxide
```
...
list("RMSE" = tree.reg.rmse,
     "Class Results" = tree.class.res)
```  
...
rf.reg.rmse <- mean((rf.reg.pred - tst$quality)^2)
...
knn.reg.rmse <- mean((knn.reg.pred - tst$quality)^2)
