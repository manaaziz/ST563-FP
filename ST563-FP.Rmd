...
tree.reg <- train(form, data = trn, method = "rpart",
                  trControl = trctrl)

# Obtain RMSE and results
tree.reg.pred <- predict(tree.reg, newdata = tst)
tree.reg.rmse <- mean((tree.reg.pred - tst$quality)^2)

# Print out tree visualization
tree.reg.out <- rpart(form, data = trn, cp = 0.01798144)
rpart.plot(tree.reg.out, main="Regression Tree")

#### HOTEL 2 ####
# Fit classification tree model
tree.class <- train(form, data = trn, method = "rpart",
                    trControl = trctrl)
...
# Print out tree visualization
tree.class.out <- rpart(form, data = trn, cp = 0.01798144)
...
rf.reg <- train(form.rf, data = trn, method = "rf",
...
rf.class <- train(form.rf, data = trn, method = "rf",
                trControl = trctrl, importance = TRUE)
...
```{r svm}
#### CLASSIFICATION ####
# Define formula for SVM model
form.svm <- 

# Fit support vector machine for H1
svm.class <- train(form.svm, data = trn, method = "svmRadial", 
...
```{r knn}
#### REGRESSION ####
# Define formula for KNN model
form.knn <- 

# Fit knn model for H1
knn.reg <- train(form.knn, data = trn, method = "knn",
...
knn.class <- train(form.kn, data = trn, method = "knn",
                   trControl=trctrl, preProcess = c("center", "scale"))
