...
```{r corrplot, echo = FALSE, out.height='3.5in'}
...
tree.class.test_err
```
The resulting classification tree creates splits on 4 variables, Alcohol, Total Sulfur Dioxide, Sulphates, and Volatile Acidity. There are 10 terminal nodes. The tree has a test error rate of 0.297. This test rate is fairly low, but as mentioned before, if one conducts pruning on the tree, the error rate can decrease and a more accurate model is created. 

Pruning is performed on the tree using cross validation, the tree is built many times with a different number of splits and the tree that produces the smallest error rate is selected. Cross validation on this model shows that the best model is the model built with 8 splits. The resulting pruned model will now have 8 terminal nodes and still uses the four variables in the un-pruned tree.    
```{r pruned tree}
...
```
The pruned tree produces a test error rate to the original tree with a test MSE of 0.3. The pruned tree would be selected using the concept of Occam's Razor, where the simplest model is the best model choice.

Using a Random Forest tree can often improve upon the error rate of a simple classification tree. While the test MSE was small in the classification tree, the team believed the error rate could be decreases in using a more complex tree based method.  
...
The first thing that should be done in building a boosted tree is cross validation. Here the model is built many times with the number of predictors included from one to all predictors. The test MSE is evaluated for each and the smallest value is selected. A plot of these is included below, the tree that produced the smallest error rate was the tree built with two variables, so that was the tree selected through cross validation.  
...
rf.class.test_err
```
Once the Random Forest tree with two variables was built, the final test MSE was calculated. This model produced a test MSE of 0.197, which is significantly lower than the test MSE for the classification model (0.3). The Random Forest model was selected as the best method for predicting if a wine were to fall into the "high" or "low" class when it comes to wine quality.  
...
```{r lm.diag, out.height = '3.5in'}
# Look at residual plot for nonlinearity
regres <- ggplot(data = data.frame(lm.reg$residuals, lm.reg$fitted.values), 
                 aes(x = lm.reg$residuals, y = lm.reg$fitted)) + 
            geom_point(position = 'jitter') + labs(x = "Residuals", y = "Fitted Values") 
...
df.class <- rbind.data.frame(prune.tree.class.test_err, rf.class.test_err) %>% round(4)
colnames(df.class) <- c("Misclassification Rate")
rownames(df.class) <- c("Classification Model", "Random Forest")
df.class
```
