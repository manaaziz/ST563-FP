...
The resulting classification tree creates splits on 4 variables: Alcohol, Total Sulfur Dioxide, Sulphates, and Volatile Acidity. The tree has a test error rate of `r round(tree.class.test_err, 3)`. This test rate is fairly low, but as mentioned before, if one conducts pruning on the tree, the error rate can decrease and a more accurate model is created. 
...
The pruned tree produces a test error rate to the original tree with a misclassification error of `r prune.tree.class.test_err`.Using a Random Forest tree can often improve upon the error rate of a simple classification tree. While the test MSE was small in the classification tree, the team believed the error rate could be decreased using a more complex tree based method.  
...
matplot(1:mtry, test.err, pch = 1, type = 'b',
        xlab = "mtry", ylab = 'Mean Square Error')
...
```{r mlr}
#### REGRESSION ####
lm.reg <- lm(quality ~ . - class, data = trn)
...
# Plot cross validation results
matplot(1:mtry, test.err, pch = 1, type = 'b', ylab = 'Mean Square Error', xlab = "mtry")
points(which.min(test.err), test.err[which.min(test.err)], pch = 8, col = "#CC0000")
...
df.reg <- rbind.data.frame(lm.reg.rmse, rf.reg.test_err) %>% round(4)
df.reg$method <- reg
...
```{r reg.select, out.height='3.5in', eval = F}
