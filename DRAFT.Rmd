...
The research question of interest for this study is whether or not machine learning methods can accurately predict the quality of a wine, independent of using taste testers. In the classification setting, we hypothesize that the machine learning methods can correctly classify wines as "high" or "low" quality with 80% accuracy. In the context of regression, we hypothesize that the regression techniques used will be able to achieve a low root mean squared error. In both settings, though, we expect the random forest to outperform the other methods.
...
The two groups were similar in most of the variables. Two notable differences are that the wines classified as high quality had a lower mean value than low quality wines for total sulfur dioxide. High quality wines had a mean of 39, which was significantly lower than the mean of the low quality wines at 54.6. Another difference to note is the difference in the mean for alcohol. The high quality wines had a mean alcohol content at 10.8, while the low quality wines had a lower mean alcohol content around 9.9. There were also differences in fixed acidity, with the lower quality wines having a lower mean. There were also several variables that don't seem to change between the two groups including pH, residual sugar, chlorides, and density. 
...
Based on cross validation, we chose our random forest model for regression to have two randomly-selected variables considered at each split. 
# Results  
...
Based on the output, the best model for both classification and regression was the random forest. This supports our hypothesis that the random forest would outperform the other methods. This was not necessarily a surprise, since the ensemble methods tend to outperform regular methods. As an average of many classification trees, ensemble methods generally improve prediction accuracy and reduce overall variance. We suspect that other ensemble methods, such as bagging or boosting, may also be able to yield higher prediction accuracy.   

In the regression setting, the values of root mean squared error (RMSE) for both models were relatively low. The RMSE for the random forest was `r df.reg[2,2]` and the RMSE for the multiple linear regression model was `r df.reg[1,2]`, further supporting our hypothesis that the random forest would outperfrom the other models.
...
Based on the results of our study, we were able to predict the high or low quality of a given wine with about 70-80% accuracy. Despite not being able to achieve 80% prediction accuracy with the basic classification tree, we *were* able to with the random forest. Although this is a *good* result considering the non-information rate of 53%, it is by no means excellent. These results indicate that machine learning algorithms could be a good technique when trying to predict wine quality, but that there is room for improvement. With a more formal, industry-expert-guided variable selection process, we believe that higher prediction accuracies can be achieved. Machine learning models could potentially be combined with other techniques to add value to winemakers.
