...
tree.class <- train(form.class, data = trn, method = "rpart",
                    trControl = trctrl, tuneGrid = expand.grid(cp = seq(0, .02, .001)))

# Plot the accuracy as a function of the tuning parameter
plot(Accuracy ~ cp, data = tree.class$results, xlab = "Cp")
points(which.max(tree.class$results$Accuracy),
       tree.class$results$Accuracy[which.max(tree.class$results$Accuracy)],
       pch = 8, col = "#CC0000")
```
...
tree.class.out <- rpart(form.class, data = trn, cp = 0.011)
...
The following parameters need to be tuned in a random forest model:  
  * `mtry`: the number of randomly-selected variables selected at a node split
  * `ntree`: the number of trees to grow

```{r rf, out.height='3.5in'}
#### REGRESSION ####
# Set seed for reproducibility
set.seed(702)
...
# Plot the RMSE and accuracy as a function of mtry (ntree constant at 500)
par(mfrow = c(1,2))
plot(RMSE ~ mtry, data = rf.reg$results)
points(which.min(rf.reg$results$RMSE), rf.reg$results$RMSE[which.min(rf.reg$results$RMSE)],
       pch = 8, col = "#CC0000")
plot(Accuracy ~ mtry, data = rf.class$results)
points(which.max(rf.class$results$Accuracy),
       rf.class$results$Accuracy[which.max(rf.class$results$Accuracy)],
       pch = 8, col = "#CC0000")
```  

Since `ntree` will plateau with enough trees, we tuned the `mtry` in both the regression and classification settings. For regression, the optimal `mtry` value is `r rf.reg$bestTune[1,1]`, while for classification it is  `r rf.class$bestTune[1,1]`. Keep in mind that the random forest is equivalent to bagging when the number of randomly selected features is equal to the total number of variables in the model. In both of the above plots, this is represented by the maximum displayed value of `mtry`.
